...
### M-Scale: スケーリング盤面 + スキルツリー + レスポンシブUI ✅（2026-05-28）

**盤面スケーリング**
- [x] `BOARD_STAGES` 定数: ターン0→16×12 / ターン10→24×18 / ターン20→32×24 / ターン35→40×30
- [x] `checkBoardExpansion()` + `applyBoardExpansion()`: グリッドを中央寄せで移植、renderer リセット
- [x] 拡大時に「🌍 盤面拡大！」スパイクテキスト演出

**スキルツリー式パターン解放**
- [x] `state.unlockedPatterns`（Set）+ `state.unlockedAttacks.area` でロック状態管理
- [x] 初期解放: block / blinker / 通常撃破のみ
- [x] `UNLOCK_ITEMS` 定数: glider(T3) / 範囲撃破(T5) / lwss(T8) / immortal(T12) / breeder(T18)
- [x] `refreshToolUI()`: ロック中ボタンをグレーアウト + dashed border
- [x] `buildUnlockPool()` in `shop.js`: 未解放かつターン条件を満たすアイテムを商人プールに動的追加

**脅威スケーリング**
- [x] `updateThreatLevel(turn)`: T5でハザード出現、T10で5ターン周期ON、T20で猛攻、T30/T40でHELL化
- [x] 序盤（T1〜4）はハザード完全無効で生存に集中できる設計

**レスポンシブUI**
- [x] CSS `clamp()` でフォント・パディング・ギャップを全面スケール
- [x] `≥900px`: CSS Grid 2カラム（盤面左・操作パネル右）
- [x] `≤480px`: タイトル非表示・ステータスコンパクト化
- [x] タッチ操作（`touchstart` / `touchmove` / `touchend`）でモバイルからも配置・撃破可能
...
- [x] ~~**モバイル対応は当面しない**~~ → タッチ操作・レスポンシブUI 対応済み（M-Scale）
...
### 2026-05-28 — スケーリング盤面 + スキルツリー + レスポンシブUI（M-Scale）

盤面が序盤16×12から終盤40×30に自動拡大。商人からパターンを解放するスキルツリー式に変更し、初見プレイヤーが段階的に複雑さを理解できる設計に。CSSを`clamp()`ベースに全面改修しマルチ解像度対応・タッチ操作を追加。ハザードも序盤無効→T5以降段階的に地獄化するスケーリング実装。
